...
def list_my_orders(ctx: AuthContext) -> dict[str, Any]:
    ...
    if ctx.role == "merchant" and store_id is None:
        return {
            "ok": False,
            "error": "invalid_argument",
    ...
    with db.connection() as conn:
        if store_id is not None and ctx.role == "merchant":
...
def find_order(ctx: AuthContext, query: str) -> dict[str, Any]:
    ...
    with db.connection() as conn:
        if ctx.role == "shopper":
            ...
            if store_id is None:
                return permission_denied(
                    f"merchant (user {ctx.user_id}) has no store to search"
                )
    ...
    for order in candidates:
        ...
        if title is None:
            continue
        if fuzz.partial_ratio(needle, title) >= FUZZY_MATCH_THRESHOLD:
